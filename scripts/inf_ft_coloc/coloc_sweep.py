#!/usr/bin/env python3
"""The colocation study driver: the split-LMCache stack + a fine-tune neighbor on GPU1.

Adapted copy of scripts/inference/split_simple/disagg_sweep.py (the lmcache stack only;
the stack scripts live in this directory). Same reference workload -- `workload.py` and
...
_ACTIVE_STACK = ["lmcache"]   # which stack the stall watchdog should tear down
...
# Only the lmcache stack exists in this study; its scripts are local copies.
STACKS = {
    "lmcache": (HERE / "lmc_launch.sh", HERE / "lmc_stop.sh"),
}
...
def start_ft(name, args):
    ...
    if args.ft_mps_pct:
        # Spatial cap via MPS: the trainer's context gets this % of SM threads.
        # Only meaningful with nvidia-cuda-mps-control running BEFORE the stack
        # launches (all GPU1 processes must be MPS clients to co-reside).
        env["CUDA_MPS_ACTIVE_THREAD_PERCENTAGE"] = str(args.ft_mps_pct)
    cmd = [str(REPO / ".venv" / "bin" / "python"), str(HERE / "ft_train.py"),
           "--out", str(ft_csv), "--model", args.ft_model,
           "--batch", str(args.ft_batch), "--seq", str(args.ft_seq),
           "--mem-cap-gib", str(args.ft_mem_cap_gib), "--duty", str(args.ft_duty)]
    if args.ft_gate:
        cmd.append("--gate")
    if args.ft_kgate:
        # kernel-level gate: CUPTI callback library in the trainer process; the
        # python block hooks are NOT used (the .so gates every launch itself).
        env["COLOC_ROLE"] = "be"
        env["LD_PRELOAD"] = str(HERE / "orion_gate" / "cupti_gate.so")
    proc = subprocess.Popen(cmd, env=env, stdout=log, stderr=subprocess.STDOUT)
    return proc, ft_csv


def ft_summary(ft_csv, t0=None, t1=None):
    """Step-time stats from the trainer's CSV, optionally windowed to [t0, t1]."""
    try:
        rows = [r for r in csv.DictReader(open(ft_csv))]
    except OSError:
        return {"error": "no ft csv"}
    if t0 is not None:
        rows = [r for r in rows if t0 <= float(r["ts"]) <= (t1 or float("inf"))]
    if not rows:
        return {"steps": 0}
    dts = sorted(float(r["dt_ms"]) for r in rows)
    out = {"steps": len(rows),
           "step_ms_p50": round(dts[len(dts) // 2], 1),
           "step_ms_mean": round(sum(dts) / len(dts), 1),
           "step_ms_p95": round(dts[int(len(dts) * 0.95)], 1),
           "tok_per_s_mean": round(sum(float(r["tok_per_s"]) for r in rows) / len(rows)),
           "peak_gib": max(float(r["peak_gib"]) for r in rows)}
    if "gated_ms" in rows[0]:
        out["gated_ms_mean"] = round(
            sum(float(r["gated_ms"]) for r in rows) / len(rows), 1)
    return out


def run_one(budget, wl, args):
    util = round(budget / GPU_TOTAL_GIB, 4)
    name = f"coloc_{args.stack}_{wl.skew}_b{budget:g}{args.name_suffix}"
    print(f"\n=== {name}  (decode util={util}) ===", flush=True)
    record = {"name": name, "budget_gib": budget, "util": util, "skew": wl.skew,
              "setup": f"coloc_{args.stack}" + ("_ft" if args.ft else ""),
    ...
    mon = subprocess.Popen([sys.executable, str(REPO / "scripts/common/gpu_monitor.py"),
                            str(mon_path)])
    ft_proc, ft_csv = None, None
    try:
        t0 = time.time()
        _ACTIVE_STACK[0] = args.stack
        env_extra = {"FORWARD_FIRST_TOKEN": "1"} if args.forward_first_token else {}
        if args.ft_gate or args.ft_kgate:
            # hp side of the idle-window gate: sitecustomize wraps execute_model in
            # the decode engine and publishes the busy window to /dev/shm.
            patch_dir = str(HERE / "orion_gate" / "hp_patch")
            old_pp = os.environ.get("PYTHONPATH", "")
            env_extra["PYTHONPATH"] = f"{patch_dir}:{old_pp}" if old_pp else patch_dir
            env_extra["COLOC_HP_SIGNAL"] = "1"
        ...
        # The neighbor starts only after the decode engine has claimed its full VRAM
        # budget, so the trainer can never race vLLM's profiling allocation.
        if args.ft:
            ft_proc, ft_csv = start_ft(name, args)
            record["ft_csv"] = str(ft_csv)
            print(f"  ft neighbor up (pid {ft_proc.pid}) -> {ft_csv.name}", flush=True)
        ...
        if ft_proc is not None:
            died_early = ft_proc.poll() is not None
            ft_proc.terminate()
            try:
                ft_proc.wait(timeout=30)
            except subprocess.TimeoutExpired:
                ft_proc.kill()
            record["ft"] = {
                "died_early": died_early,
                "overall": ft_summary(ft_csv),
                "measure_window": ft_summary(ft_csv, record.get("t_measure0"),
                                             record.get("t_measure1")),
            }
            fw = record["ft"]["measure_window"]
            print(f"  ft: {record['ft']['overall'].get('steps', 0)} steps total, "
                  f"measure window p50 {fw.get('step_ms_p50')} ms "
                  f"({fw.get('steps', 0)} steps)"
                  f"{'  FT DIED EARLY' if died_early else ''}", flush=True)
        mon.terminate()
        stop(args.stack)
...
def derive_row(rec, wl):
    ...
    return {
        ...
        "tpot_p50_ms": s.get("tpot_ms", {}).get("p50"),
        "tpot_p95_ms": s.get("tpot_ms", {}).get("p95"),
        "tpot_mean_ms": s.get("tpot_ms", {}).get("mean"),
        ...
        "ft": bool(rec.get("ft")),
        "ft_steps_measure": rec.get("ft", {}).get("measure_window", {}).get("steps"),
        "ft_step_ms_p50": rec.get("ft", {}).get("measure_window", {}).get("step_ms_p50"),
        "ft_tok_per_s": rec.get("ft", {}).get("measure_window", {}).get("tok_per_s_mean"),
        "ft_peak_gib": rec.get("ft", {}).get("overall", {}).get("peak_gib"),
        **rec.get("gpu", {}),
...
def main():
    ...
    ap.add_argument("--stack", choices=list(STACKS), default="lmcache")
    ap.add_argument("--ft", action="store_true",
                    help="run the fine-tune neighbor on the decode GPU for the whole run")
    ap.add_argument("--ft-model", default="Qwen/Qwen2.5-0.5B")
    ap.add_argument("--ft-batch", type=int, default=2)
    ap.add_argument("--ft-seq", type=int, default=512)
    ap.add_argument("--ft-mem-cap-gib", type=float, default=4.5)
    ap.add_argument("--ft-duty", type=float, default=1.0)
    ap.add_argument("--ft-mps-pct", type=int, default=0,
                    help="CUDA_MPS_ACTIVE_THREAD_PERCENTAGE for the trainer (0 = unset); "
                         "requires an MPS daemon and CUDA_MPS_PIPE_DIRECTORY in the env")
    ap.add_argument("--ft-gate", action="store_true",
                    help="idle-window gating at block granularity: decode engine "
                         "publishes its busy window (orion_gate/hp_patch via "
                         "PYTHONPATH), trainer issues kernels only in the idle gaps "
                         "(ft_train --gate)")
    ap.add_argument("--ft-kgate", action="store_true",
                    help="idle-window gating at KERNEL granularity: same hp signal, "
                         "but the trainer is gated per launch by the CUPTI callback "
                         "library (orion_gate/cupti_gate.so) — catches cuBLAS too")
