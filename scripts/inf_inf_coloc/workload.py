...
# Qwen2.5-0.5B (bf16): 24 layers * 2 kv heads * 64 dim * 2 (K,V) * 2 bytes = 12 KiB/token
# -- ~10.7x lighter than the 8B, which is what makes a 3x-oversubscribed tenant fit in
# ~1 GiB of KV grant.
QWEN_KV_BYTES_PER_TOKEN = 24 * 2 * 64 * 2 * 2  # 12288
GIB = 1 << 30
...
class Workload:
    ...
    def working_set_gib(self) -> float:
        return self.working_set_tokens * self.kv_bytes_per_token / GIB

    @property
    def prefix_gib(self) -> float:
        """KV of a single session prefix -- the unit that moves over PCIe on a DRAM hit."""
        return self.prefix_len * self.kv_bytes_per_token / GIB
...
def build_workload(
    ...
    return Workload(sessions, requests, num_sessions, prefix_len, suffix_len,
                    skew, zipf_a, seed, kv_bytes_per_token)
...
if __name__ == "__main__":
    ...
    print("8B reference:", json.dumps(s, indent=2))
    for sessions, label in ((8, "qwen fits"), (48, "qwen offload")):
        wl = build_workload(sessions, seed=1000,
                            kv_bytes_per_token=QWEN_KV_BYTES_PER_TOKEN)
        s = wl.summary()
        s.pop("session_request_counts")
        print(f"{label}:", json.dumps(s, indent=2))
