...
class Pipeline():
    ...
    def init_streams(self):
        ...
        # size SM partitions to the actual device (upstream hardcoded H100's 132)
        self.total_sm = torch.cuda.get_device_properties(self.device).multi_processor_count
        upper = ((self.total_sm - 4) // 8) * 8  # paired splits sum to `upper`, remainder stays free
        self.sm_counts = [
            i for i in range(8, upper, 8) # Assuming SM counts are in increments of 8
        ]
        # H100 (132 SM): [8, 16, ..., 120]; RTX 5090 (170 SM): [8, 16, ..., 152]
    ...
    def init_external_data(self):
        ...
        import os
        kv_pages = int(os.environ.get("NANOFLOW_KV_PAGES", 2048 * 26))
        self.kv_pool = DistKVPool(self.num_layers, self.num_kv_heads, self.head_dim, kv_pages, self.page_size, 1, self.device)
    ...
    def config_algorithm(self):
        ...
        if self.is_auto_search_enabled:
            ...
            def _tag(op, tag):
                # nano-split ops need one tag per nano op
                if getattr(op, "isNanoSplit", False):
                    op.config_tag([tag] * len(op.nano_ops), params)
                else:
                    op.config_tag(tag, params)

            _tag(self.layerNormAttn, "cuda")
            _tag(self.kqv, "torch")
            _tag(self.ropeAppend, "cuda")
            _tag(self.decAttn, "batched_cuda")
            _tag(self.pfAttn, "batched_cuda")
            _tag(self.layerNormFFN, "cuda")
            _tag(self.o, "torch")
            _tag(self.ug, "torch")
            _tag(self.activation, "cuda")
            _tag(self.d, "torch")
    ...
    def config_streams(self):
        ...
        if self.is_auto_search_enabled:
            ...
            def _set(op, stream_tuple):
                # nano-split ops need one stream per nano op
                if getattr(op, "isNanoSplit", False):
                    op.set_stream([stream_tuple] * len(op.nano_ops))
                else:
                    op.set_stream(stream_tuple)

            # optional overlap mode: run decode attention (MEM-bound) on a green
            # context SM partition, compute ops on the complementary partition
            comp_sm = int(os.environ.get("NANOFLOW_COMP_SM", "0"))
            mem_sm = int(os.environ.get("NANOFLOW_MEM_SM", "0"))
            comp_stream = self.streams[CategoryType.COMP][comp_sm] if comp_sm else (self.main_stream, self.total_sm)
            mem_stream = self.streams[CategoryType.MEM][mem_sm] if mem_sm else (self.main_stream, self.total_sm)

            for op in [self.layerNormAttn, self.kqv, self.ropeAppend, self.pfAttn,
                       self.layerNormFFN, self.o, self.ug, self.activation, self.d]:
                _set(op, comp_stream)
            _set(self.decAttn, mem_stream)
