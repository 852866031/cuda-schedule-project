...
**Inputs and offline profiling.** For every online job, offline job, and representative kernel, MMK records its proposed MIG size and MPS limit; average SM, memory-bandwidth, and L2 use; and, for kernels, grid/block dimensions, register count, and shared-memory use. It also records the corresponding features of jobs already sharing the candidate MIG instance. These samples train a random-forest predictor. At runtime, the same measurements plus the proposed colocated set form the input to the predictor; its output is **predicted execution time** for an online job or individual kernel, and **predicted throughput** for an offline job. Thus, *offline profiling* supplies the learned model, whereas an *offline job* is simply a background throughput workload.

**Choosing a MIG layout.** MMK does not ask the predictor to directly output a MIG placement. It first computes **Partition Entropy** from each job's multidimensional resource-demand vector. This is a pruning heuristic: it chooses a promising partition granularity and retains only legal MIG layouts at that granularity, instead of exhaustively searching every MIG shape and placement. For each retained layout, MMK groups the jobs into its MIG instances, computes a candidate MPS configuration inside every instance, and asks the predictor to estimate the resulting throughput. It selects the MIG/MPS candidate with the highest estimated system throughput. Since changing MIG is disruptive, this search is acted upon only in a safe window, for example when the affected partition has no running online job or an empty run queue.

**Choosing MPS limits.** Given one candidate MIG layout, MMK separates online and offline jobs and starts from baseline MPS shares that divide the instance's nominal compute capacity. It then adds a **kernel-aware oversubscription** increment: if the other colocated jobs exhibit highly variable aggregate kernel utilization, their troughs imply temporary slack, so the target job receives a higher MPS ceiling. The resulting percentages can exceed 100% in total; this does not reserve more SMs than exist, but allows a job to use slack when others are below their ceilings. MMK evaluates the resulting offline throughput through the predictor and uses it to compare candidate configurations. MPS reconfiguration is much less frequent than kernel scheduling, and MMK avoids disrupting online clients.

**What interception adds.** After MIG placement and MPS ceilings establish the relatively slow-changing resource envelope, the kernel scheduler intercepts CUDA launches and holds them in software before releasing them to the real driver. It uses predicted per-kernel time, resource contention, and the remaining online QoS slack to decide whether a *whole* kernel may launch now. For online-versus-offline sharing, it admits offline kernels only while their accumulated predicted time fits within the online slack-derived budget; otherwise it delays later offline kernels. It similarly admits online concurrency only when the predicted interference fits within slack, and packages compatible offline kernels when no strict online QoS constraint is active. Interception therefore does **not** choose the MIG layout or rewrite kernels: it enforces short-term launch timing within the MIG/MPS envelope selected above.
...
- **Online vs. online:** MMK computes the minimum slack, `min over online jobs (QoS − observed execution time)`, and admits additional online execution through MPS only when predicted execution and contention fit inside that safety margin.
- **Online vs. offline:** it converts minimum online slack into an offline execution budget, approximately `S_min × offline MPS quota`, and admits predicted offline-kernel durations until that budget is exhausted. Further offline launches wait while online work is at risk.
